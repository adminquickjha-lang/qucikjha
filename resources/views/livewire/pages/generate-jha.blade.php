...
new #[Layout('layouts.safety')] class extends Component {
    ...
    public function getTypeInfoProperty()
    {
        ...
        return $info[strtolower($this->type)] ?? $info['jha'];
    }
    ...
    public function regulations()
    {
        return [
            ...
            'Australia' => [
                ['id' => 'aus-whs', 'label' => 'Australia WHS Regulations', 'desc' => 'work health and safety regulations'],
    ...
    public function generate()
    {
        ...
        // Temp unsaved model — agent uses its properties to build the prompt
        ...
        try {
            $agent = new \App\Ai\Agents\JhaAgent($tempDoc, $regulationsStr, $extraContext);

            $aiResponse = null;
            $lastException = null;
            for ($attempt = 1; $attempt <= 3; $attempt++) {
                try {
                    $aiResponse = $agent->prompt("Please generate the JHA JSON now based on the provided context.", attachments: $attachments);
                    break;
                } catch (\Exception $e) {
                    $lastException = $e;
                    if ($attempt < 3) {
            ...
            \Illuminate\Support\Facades\Log::info('JHA AI Raw Response:', ['content' => $content]);
            ...
            $steps = $decoded['steps'] ?? $decoded['jha_steps'] ?? $decoded['jsa_steps'] ?? $decoded['job_steps'] ?? $decoded['activity_steps'] ?? $decoded['safety_steps'] ?? null;

            if (!$decoded || empty($steps) || !is_array($steps)) {
                \Illuminate\Support\Facades\Log::error('JHA Parsing Failed. Decoded Keys:', ['keys' => $decoded ? array_keys($decoded) : 'null']);
                if ($logoPath)
            ...
            if ($logoPath)
                \Illuminate\Support\Facades\Storage::disk('public')->delete($logoPath);
            \Illuminate\Support\Facades\Log::error('JHA Creation Error: ' . $e->getMessage());
            $this->dispatch('swal', [
        ...
        session()->flash('success', 'JHA document created successfully!');

        return $this->redirectRoute('preview.jha', ['id' => $doc->id]);
    }
...
<div class="relative min-h-screen pt-8 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    ...
    <form wire:submit.prevent="generate" class="lg:grid lg:grid-cols-12 lg:gap-10 items-start">
        <!-- Left Column: Core Form (Contact-Us Minimalist Style) -->
        <div class="lg:col-span-8 space-y-8">
            <div
                ...
                <!-- Field-to-field gap reduced, contact form look -->
                <div class="grid md:grid-cols-2 gap-4 mb-6">
                    <div class="space-y-1.5">
                        <!-- Label: 14px, dark, capitalized first word -->
                        <label class="text-[14px] font-semibold text-slate-900 ml-1">Project name <span
                        ...
                        <input wire:model="projectName"
                            ...
                            placeholder="e.g. infrastructure development" required />
            ...
            <div
                ...
                <div
                    ...
                    @if ($logo && method_exists($logo, 'temporaryUrl'))
                        ...
                        <span class="text-[9px] font-bold text-primary mt-2 uppercase tracking-widest">replace logo</span>
                    @else
