...
class AdobePdfService
{
    ...
    public function convertPdfToDocx($pdfPath)
    {
        ...
        if ($assetResponse->failed()) {
             \Illuminate\Support\Facades\Log::error('Adobe Asset Creation Failed Body: ' . $assetResponse->body());
        ...
        \Illuminate\Support\Facades\Log::info('Adobe Asset Created: ' . $assetId);
        ...
        if ($uploadResponse->failed()) {
            \Illuminate\Support\Facades\Log::error('Adobe Upload Failed: ' . $uploadResponse->status() . ' ' . $uploadResponse->body());
        ...
        if (!in_array($jobResponse->status(), [201, 202])) {
            \Illuminate\Support\Facades\Log::error('Adobe Export Job Failed (' . $jobResponse->status() . '): ' . $jobResponse->body());
        ...
        if (!in_array($jobResponse->status(), [201, 202])) {
            \Illuminate\Support\Facades\Log::error('Adobe Export Job Retry Failed (' . $jobResponse->status() . '): ' . $jobResponse->body());
        ...
        while ($retryCount < $maxRetries) {
            ...
            if ($statusCheck->failed()) {
                \Illuminate\Support\Facades\Log::error('Adobe Status Check Failed (' . $statusCheck->status() . '): ' . $statusCheck->body());
            ...
            \Illuminate\Support\Facades\Log::info('Adobe Job Status (Attempt ' . ($retryCount + 1) . '): ' . $currentStatus);
            ...
            if ($currentStatus === 'failed' || $currentStatus === 'error') {
                \Illuminate\Support\Facades\Log::error('Adobe Job Failed Response: ' . json_encode($statusData));
